...
public class XposedUniversalCopy implements IXposedHookLoadPackage {
  ...
  @Override public void handleLoadPackage(XC_LoadPackage.LoadPackageParam loadPackageParam)
    ...
    //优化QQ 下的体验。
    if (loadPackageParam.packageName != null && loadPackageParam.packageName.startsWith(
        "com.tencent.mobileqq")) {
      mFilters.add(new Filter.QQSingleLineTextView(loadPackageParam.classLoader));
      mFilters.add(new Filter.QQZoneFeedForwardTextView(loadPackageParam.classLoader));
      mFilters.add(new Filter.QQZonePraiseListView(loadPackageParam.classLoader));
    }
    //优化微信 下的体验。
    if (loadPackageParam.packageName != null && loadPackageParam.packageName.startsWith(
        "com.tencent.mm")) {
...
//      }
      ...
      if (appXSP.getBoolean(MONITOR_LONG_CLICK, true)) {
        findAndHookMethod(Activity.class, "onKeyLongPress", int.class, KeyEvent.class,
            new ViewonKeyLongPressHooker());
      }
      if (appXSP.getBoolean(MONITOR_CLICK, true)) {
        findAndHookMethod(Activity.class, "onKeyUp", int.class, KeyEvent.class,
            new ViewonKeyUpHooker());
      }
    }
  }
  ...
  private void setXpoedEnable(XC_LoadPackage.LoadPackageParam loadPackageParam)
    ...
    if (loadPackageParam.packageName.equals(PACKAGE_NAME)) {
      findAndHookMethod(
          loadPackageParam.classLoader.loadClass("com.pl.universalcopy.xposed.XposedEnable"),
          "isEnable", new XC_MethodReplacement() {
  ...
  private class UniversalCopyActivityThreadHook extends XC_MethodHook {

    @Override protected void afterHookedMethod(MethodHookParam param) throws Throwable {
      super.beforeHookedMethod(param);
      mUniversalCopyHandler.setActivityThreadObject(param.thisObject);
    }
  }
